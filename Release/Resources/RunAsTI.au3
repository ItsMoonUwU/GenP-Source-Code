...
If $processPID = -1 Then
	ConsoleWrite("Return 0 ; failed to get winlogon PID in current session")
	Exit
EndIf
...
;Set command line
$sCmdLine = $sModuleName & " svchost.exe " & $dwSessionId & " " & $sCmdLine

;Start service
run("cmd.exe /c sc start svchost", "", @SW_HIDE, 0x10000)
